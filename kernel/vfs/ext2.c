...
u32 init_ext2(u32 base){
    ...
    struct ext2_base_information        * ext2_basicinf;
    struct file_system_type             * ext2_type;
    struct super_block                  * ext2_superblock;
    struct dentry                       * ext2_root_dentry;
    struct inode                        * ext2_root_inode;
    struct vfsmount                     * ext2_root_mnt;

    // 构建 ext2_basic_information 结构
    ext2_basicinf = (struct ext2_base_information *) kmalloc ( sizeof(struct ext2_base_information) );
    ext2_basicinf->ex_base = base;
    ext2_basicinf->ex_first_sb_sect = ext2_basicinf->ex_base + EXT2_BOOT_BLOCK_SECT;
    
    // 构建 ext2_super 结构
    ext2_basicinf->sb.data = (u8 *) kmalloc ( sizeof(u8) * EXT2_SUPER_BLOCK_SECT * SECTOR_SIZE );
    if (ext2_basicinf->sb.data == 0)
        return -ENOMEM;
    err = read_block(ext2_basicinf->sb.data, ext2_basicinf->ex_first_sb_sect, EXT2_SUPER_BLOCK_SECT);
    if (err)
        return -EIO;

    // 找到块描述符表的首地址（绝对扇区地址）
    if (ext2_basicinf->sb.attr->block_size == 0 || ext2_basicinf->sb.attr->block_size == 1)
        ext2_basicinf->ex_first_gdt_sect = base + EXT2_BOOT_BLOCK_SECT + EXT2_SUPER_BLOCK_SECT;
    else
        ext2_basicinf->ex_first_gdt_sect = base + (( EXT2_BASE_BLOCK_SIZE << ext2_basicinf->sb.attr->block_size) >> SECTOR_SHIFT);
    
    // 构建 file_system_type 结构
    ext2_type = (struct file_system_type *) kmalloc ( sizeof(struct file_system_type) );
    if (ext2_type == 0)
        return -ENOMEM;
    ext2_type->name = "ext2";

    // 构建 super_block 结构
    ext2_superblock = (struct super_block *) kmalloc ( sizeof(struct super_block) );
    if (ext2_superblock == 0)
        return -ENOMEM;
    ext2_superblock->s_dirt    = S_CLEAR;
    ext2_superblock->s_blksize = EXT2_BASE_BLOCK_SIZE << ext2_basicinf->sb.attr->block_size;
    ext2_superblock->s_type    = ext2_type;
    ext2_superblock->s_root    = 0;
    ext2_superblock->s_fs_info = (void*)ext2_basicinf;
    ext2_superblock->s_op      = &ext2_super_operations;
    ...
    ext2_root_dentry->d_sb              = ext2_superblock;
    ...
    ext2_superblock->s_root = ext2_root_dentry;
    ...
    ext2_root_inode->i_sb               = ext2_superblock;
    ext2_root_inode->i_blksize          = ext2_superblock->s_blksize;
    ...
    ext2_root_inode->i_data.a_pagesize  = ext2_superblock->s_blksize;
    ...
    ext2_root_mnt->mnt_sb            = ext2_superblock;
...
u32 ext2_delete_inode(struct dentry *dentry){
    ...
    // 清除块位图上所有相关的位
    ext2_basicinf             = (struct ext2_base_information *) inode->i_sb->s_fs_info;
    inodes_per_group    = ext2_basicinf->sb.attr->inodes_per_group;
    blocks_per_group    = ext2_basicinf->sb.attr->blocks_per_group;
    ...
    sect = inode_table_base + (inode->i_ino - 1) % inodes_per_group / ( SECTOR_SIZE / ext2_basicinf->sb.attr->inode_size);

    err = read_block(buffer, sect, 1);
    if (err)
        return -EIO;
    
    kernel_memset(buffer + ((inode->i_ino - 1) % inodes_per_group % ( SECTOR_SIZE / ext2_basicinf->sb.attr->inode_size)) * ext2_basicinf->sb.attr->inode_size, \
                    0, ext2_basicinf->sb.attr->inode_size);
...
u32 ext2_write_inode(struct inode * inode, struct dentry * parent){
    ...
    sect                = group_sect_base + 2 * (inode->i_blksize >> SECTOR_SHIFT) + \
                            ( (inode->i_ino - 1) % inodes_per_group ) / ( SECTOR_SIZE / ext2_basicinf->sb.attr->inode_size);
    err = read_block(buffer, sect, 1);
    if (err)
        return -EIO;
    
    ex_inode            = (struct ext2_inode *)(buffer + \
        (inode->i_ino - 1) % ( SECTOR_SIZE / ext2_basicinf->sb.attr->inode_size) * ext2_basicinf->sb.attr->inode_size );
...
u32 ext2_group_base_sect(struct inode * inode){
    ...
    // 计算inode所在的组号和组偏移
    ext2_basicinf             = (struct ext2_base_information *) inode->i_sb->s_fs_info;
    base                = ext2_basicinf->ex_base;
    blksize             = inode->i_blksize;
    inodes_per_group    = ext2_basicinf->sb.attr->inodes_per_group;
    group               = (inode->i_ino - 1) / inodes_per_group;            // 因为没有0号inode的映像
    sect                = ext2_basicinf->ex_first_gdt_sect + group / (SECTOR_SIZE / EXT2_GROUP_DESC_BYTE);
