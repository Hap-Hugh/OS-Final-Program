...
u32 init_fat32(u32 base) {
	...
	if (fat32_BI == 0)
		return -ENOMEM;
	fat32_BI->fat32_DBR = 0;
	fat32_BI->fat32_FSINFO = 0;
	fat32_BI->fat32_FAT = 0;
	...
	fat32_BI->fat32_FAT = (struct fat32_file_allocation_table *) kmalloc \
		(sizeof(struct fat32_file_allocation_table));
	if (fat32_BI->fat32_FAT == 0)
		return -ENOMEM;
	fat32_BI->fat32_FAT->base = base + fat32_BI->fat32_DBR->reserved;                 // FAT起始于非保留扇区开始的扇区

	fat32_BI->fat32_FAT->data_sec = fat32_BI->fat32_FAT->base + fat32_BI->fat32_DBR->fat_num * \
		fat32_BI->fat32_DBR->fat_size;
	fat32_BI->fat32_FAT->root_sec = fat32_BI->fat32_FAT->data_sec + \
		(fat32_BI->fat32_DBR->root_clu - 2) * fat32_BI->fat32_DBR->sec_per_clu;
	// 因为0、1号簇没有对应任何扇区
	...
	fat32_sb->s_blksize = fat32_BI->fat32_DBR->sec_per_clu << SECTOR_SHIFT;
	...
	// root_inode->i_size              = fat32_BI->fat32_FAT->fat.table[fat32_BI->fat32_DBR->root_clu].size;
	// TODO 得到i_size
	...
	i = 0;
	next_clu = fat32_BI->fat32_DBR->root_clu;
...
u32 fat32_readpage(struct vfs_page *page) {
	...
	data_base = ((struct fat32_basic_information *)(inode->i_sb->s_fs_info))->fat32_FAT->data_sec;
...
u32 read_fat(struct inode* inode, u32 index) {
	...
	base_sect = FAT32_BI->fat32_FAT->base;
