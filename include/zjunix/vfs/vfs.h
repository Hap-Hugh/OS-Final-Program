...
struct file_system_type {
    u8                                  *name;                  // 名称
	u8									*next;					// 标记下一个文件系统的名称
};

// 超级块
struct super_block {
	struct list_head					s_list;					// 用于超级块链表的指针
	u8                                  s_dirt;                 // 修改标志
	u32                                 s_blksize;              // 以字节为单位的块大小
	unsigned long long					s_max_size;				// Max File Size以字节为单位的最大文件大小
	...
	struct list_head					s_locked_inodes;		// 链接用于同步的文件的inodes
	struct list_head					s_files;				// 链接已经打开的文件                              *s_fs_info;             // 指向特定文件系统的超级块信息的指针
};
// 超级块的操作函数
struct super_operations {
	...
	// 释放指定索引节点
	void(*vfsFree_inode) (struct inode *);                      
...
struct inode_operations {
	...
	u32(*create) (struct inode *, struct dentry *, u32, struct nameidata *);			// 在某一目录下，为与目录项相关的普通文件创建一个新的磁盘索引节点
	struct dentry * (*lookup) (struct inode *, struct dentry *, struct nameidata *);	// 为包含在一个目录项对象中的文件名对应的索引节点查找目录
	u32(*mkdir) (struct inode*, struct dentry*, u32);									/* mkdir()，创建父目录inode下的一个新目录dentry，mode指定创建时的初始模式 */
	u32(*rmdir) (struct inode*, struct dentry*);										/* rmdir()，删除父目录inode中的子目录dentry */	
	u32(*rename) (struct inode*, struct dentry*, struct inode*, struct dentry*);		/* 该函数由挂载的VFS调用，重命名，前两个是原文件和原目录 */
	u32(*setMyTime) (struct inode*, struct dentry*);									/* setMyTime(), 设置时间*/
};
...
struct dentry_operations {
	...
	u32(*compare)(const struct qstr *, const struct qstr *);		/* 当目录项对象的 d_count 为0时，VFS调用这个函数 */
	int(*d_delete)(struct dentry *);								/* 当目录项对象的 d_count 为0时，VFS调用这个函数 */
	void(*d_release)(struct dentry *);								/* 当目录项对象将要被释放时，VFS调用该函数 */
};
// 打开的文件
struct file {
    u32                                 f_pos;                  // 文件当前的读写位置
